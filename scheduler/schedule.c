 ...
 */
  ...
  */
  /*
  - to rq einai desmeumenos pointer
  - to seedTask kai afto
  */
...
void schedule()
{
  ...
  int i;
  long long minexp=0;//thelei nai to arxikopoihseis me miden?
  long long maxwait=0;
  long long last_duration=0;
  ...
  if (rq->nr_running == 1) {
	...
	minexp=curr->exp_burst;//arxikopoihsh se miden...kati den maresei edw:p
      }
    ...
    if (todo==rq->head) {
      printf("################################ M P I K E INIIIIIIIIIIIIIIIIIIIIIT\n");  
    }
    //nxt=todo->next;
    //printf("\n\n");sleep(1);print_rq();sleep(1);printf("\n\n");
    context_switch(todo);
  }
...
{
	// p->time_slice = 100;..afto gia to roundRobin
        p->last_cpu_taken=0;
        p->exp_burst=0;
...
void activate_task(struct task_struct *p)
{
	...
	p->next->prev = p;
	p->prev->next = p;
	p->last_rq_enter=sched_clock();//to vala egw
	rq->nr_running++;
}
